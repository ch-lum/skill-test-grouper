...
def add_title(*args, title="Dr."):
    ...
    '''
    title_add = lambda x: title + " " + x
    return list(map(title_add, args))
...
def invite(cities, **kwargs):
    ...
    return_list = []
    for key in kwargs:
        if kwargs[key] in cities:
            return_list.append(key + " is invited")
    return return_list
...
def factorial(n):
    ...
    120
    '''
    # base case of when to exit recursive step
    if (n == 1):
        return 1
    else:
